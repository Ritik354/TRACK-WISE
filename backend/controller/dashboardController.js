...
export async function getDashboardData(req, res) {
  ...
  try{
    const incomes = await incomeModel.find({ userId, date: { $gte: startDate , $lte: endDate } }).lean();
    const expenses = await expenseModel.find({ userId, date: { $gte: startDate , $lte: endDate } }).lean();
